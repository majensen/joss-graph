...
sub AUTOLOAD {
  ...
  unless (grep /^$method/, keys %neo_queries) {
    $log->logcroak("Method '$method' not defined in pacakge ".__PACKAGE__);
  }
...
%neo_queries = (
  review_pending => <<QRY,
match (s:submission {disposition:"review_pending"}) with toInteger(replace(s.prereview_issue, "https://github.com/openjournals/joss-reviews/issues/","")) as issn return issn
QRY
  under_review => <<QRY,
